...
import { __ROOT__, __LOADER__, showToast, selector, selectorAll } from "./flo3fwf.js";
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    window.addEventListener("popstate", function (event) {
      //console.log(event);
        if (event.state && event.state.tool) {
            ...
            __LOADER__(mainContent, '.html', tool, '');
    ...
    let path = currentPath.replace('#', '').split('/');
      ...
      //check if browser url end with / using window
      // if (window.location.pathname.endsWith('/')) {
      //     // If it does, remove the trailing slash
